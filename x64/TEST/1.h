...
public:
	MertricAnalysis* metricAnalysis() { //get p_metricAnalysis pointer
		return p_metricAnalysis;
	}
	Repository(Scanner::Toker* pToker)  //set toker pointer
	{
		p_Toker = pToker;
	}
	ScopeStack<element*>& scopeStack() //get stack
	{
		return stack;
	}
	Scanner::Toker* Toker() //get toker pointer
	{
		return p_Toker;
	}
	size_t lineCount() //get line number
	{
...
{
public:
	bool doTest(ITokCollection*& pTc, AST<element>& abstractSyntaxtTree) //BeginningOfScope rule test
	{
...
public:
	...
	std::string ruleName() { // return the rule's name
		return "EndOfScope";
	}
};
...
class HandlePop : public IAction
{
	Repository* p_Repos;
public:
	HandlePop(Repository* pRepos) //constructor
	{
		p_Repos = pRepos;
	}
	void doAction(ITokCollection*& pTc, AST<element>& abstractSyntaxtTree) //EndOfScope action
	{
		//std::cout << "\n--EndOfScope rule";
		...
		if (elem->type == "function")
		{
 		...
 		//	std::cout << "\n  Function " << elem->name << ", lines = " << elem->lineCount <<", complexity = "<<elem->complexity;
			//std::cout << "\n";
			//p_Repos->metricAnalysis()->TreeWalk(p_Repos->scopeStack().top());
		}
	}
};
...
class PushSpecialKeyWord : public IAction {
	Repository* p_Repos;
public:
	PushSpecialKeyWord(Repository* pRepos) { //constructor
		p_Repos = pRepos;
	}
	...
	void doAction(ITokCollection*& pTc, AST<element>& abstractSyntaxtTree) {
		ITokCollection& tc = *pTc;
		size_t len = tc.find("(");
		p_Repos->scopeStack().top()->name = "statement";
...
class PushKeyWordWithoutParentheses : public IAction {
	Repository* p_Repos;
public:
	PushKeyWordWithoutParentheses(Repository* pRepos) { //constrcutor
		p_Repos = pRepos;
	}
...
public:
	...
	bool doTest(ITokCollection*& pTc, AST<element>& abstractSyntaxtTree) 
	{
		...
		if (tc[tc.length() - 1] == "{")
		{
			size_t len = tc.length() - 1;
			while (tc[len - 1] == "\n") 
				len--; //move to class and namespace name
			len--;       //using namespace Scaner {
			while (tc[len - 1] == "\n") // move to namespace and class type
				len--;
			if (isNameSpaceOrClass(tc[len - 1]))
			{
				//std::cout << "  KeyWordWithoutParentheses match!\n";
				doActions(pTc, abstractSyntaxtTree);
				return false;
